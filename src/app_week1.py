# TooManyRules — ЗАМОРОЖЕННЫЙ АРТЕФАКТ НЕДЕЛИ 1 (дни 1-5).
#
# Это код недели 1 в том виде, в каком он был на теге `day05`: день 1
# (чат с DeepSeek), день 2 (формат ответа A vs B), день 3 (четыре способа
# рассуждения), день 4 (три температуры × три повтора), день 5 (матрица
# «модель × thinking»). Каждый день — своя вкладка.
#
# Файл больше не редактируется и не рефакторится под архитектуру недели 2:
# он самодостаточен, ничего не импортирует из нового кода (`agent.py`,
# `presets.py`, `app.py`) и ничего им не отдаёт. Дублирование системного
# промпта, констант DeepSeek и таблицы цен с новым кодом — осознанная цена
# заморозки: артефакт должен запускаться и тогда, когда новый код уедет
# далеко вперёд (см. `docs/TooManyRules — Неделя 2 архитектура.md`, §2).
#
# Запуск: ./run.sh week1 (или python app_week1.py из src/ с активированным
# .venv — после pip install -r requirements.txt и настройки .env)
