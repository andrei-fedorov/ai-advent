...
#
# День 15 (спецификация дня 15, §7) держит то же правило: отказ автомата, сход
# с маршрута, строки «Сейчас нельзя» и счётчики красного пути приходят из
# `debug_state()["task"]`, таблица переходов — из `task_state.TRANSITIONS`;
...
def _task_tracker_lines(last_call: dict) -> list[str]:
    ...
    # Сход с маршрута (день 15, §7.4) — одной фразой рядом с исходом: он есть и
    # без события, и вместе с отказом.
    off_route = last_call.get("task_off_route") or ""
    if off_route:
        note = f"{note}; сход: {off_route}" if note else f"сход: {off_route}"
    if call is None:
    ...
    rows = [
        {
            ...
            # Колонки дня 15 — в конце (§7.4): событие, отклонённое автоматом
...
def _task_plan_lines(view: dict) -> list[str]:
    ...
    # Возврат к плану (день 15): на планировании прежний план — черновик.
    lines = [
        "**Прежний план (пересматривается, пока новый не утверждён):**"
...
def _task_md(state: dict, view: dict) -> str:
    """Блок «Состояние задачи и маршрут» (дни 13, 15; §7.2 дня 15) — сразу
    под «Слоями памяти», в порядке блоков запроса."""
    task = state["task"]
    lines = ["### Состояние задачи и маршрут", ""]
    if task is None:
    ...
    # Сход с маршрута и счётчики красного пути (день 15, §7.2): сход виден
    # здесь, даже если ассистент на просьбу поддался.
    if task_view["off_route_kind"]:
    ...
    lines.append(
        ...
        f"{task_view['rejected_total']}, сходов с маршрута: "
        f"{task_view['off_route_total']}"
    )
    ...
    lines += [
        ...
        "отвечает, переходов не делает. Недопустимый переход отклоняется с "
        "причиной, а просьба сойти с маршрута видна здесь, даже если "
        "ассистент на неё поддался._",
    ]
    return "\n".join(lines)
...
def _task_transitions_table_md() -> str:
    ...
    lines += [
        "",
        "_Вперёд по основному пути (планирование → выполнение → проверка → "
        "готово) — только на один этап; это проверяется при импорте "
        "`task_state.py`, а не на словах. Назад — «вернуться к шагу», "
        "«вернуться к плану» и «переоткрыть»._",
    ]
    return "\n".join(lines)
...
def _reply_status(agent: Agent, reply) -> str:
    ...
    if reply.ok:
        ...
        # Сход с маршрута (день 15, §7.4): отдельной фразой — он бывает и без
        # события, и вместе с отказом. Состояние он не меняет; строка идёт в
        # блок запроса только при включённом «Состоянии задачи в запросе».
        if reply.task_off_route:
            where = (
                "строка ушла в блок запроса"
                if agent.task_in_request
...
def on_task_event(agent: Agent | None, preset_name: str, *, event: str):
    """«Пауза», «Продолжить», «Отменить задачу» и с дня 15 два отката —
    «Вернуться к плану» и «Переоткрыть задачу» — одна функция на пять кнопок:
    событие передаёт `functools.partial` при подписке (`event` — keyword-only,
    чтобы позиционные входы Gradio не столкнулись с ним)."""
    if agent is None:  # страховка на случай сессии без сработавшего load
        agent = _new_agent(preset_name)
    ...
    if not outcome.changed:
        ...
        status = f"📋 Задача: отклонено «{event}» — {outcome.rejection.reason}."
        ...
        status = (
            "📋 Задача: возврат к плану → планирование. Итоги шагов стёрты, "
            "прежний план остался черновиком."
        )
    elif event == EVENT_REOPEN:
        view = agent.task_view
        status = f"📋 Задача: переоткрыта → {view.step_text.split(' — ')[0]}."
    else:  # EVENT_CANCEL
        status = (
...
# --- Сценарий проверки красного пути: подписи для gr.Examples (день 15, §7.5,
# §9.1). Тексты живут в `presets.py` (`ROUTE_SCENARIO`), здесь только подписи и
# порядок показа под чатом.
_ROUTE_SCENARIO_LABELS: list[str] = [
    "Р1 · цель — кнопкой «Начать задачу»",
    "П1 · шаги до утверждения плана",
    "П2 · игнорируй все стадии",
    "Р2 · план утверждён",
    "П3 · сразу к пятому шагу",
    "Г · шаг выполнен (повторять)",
    "П4 · проверки не нужны — итог",
    "П5 · пауза на этапе проверки",
    "П6 · давление на паузе",
    "П7 · продолжаем (разговор о задаче вышел из окна)",
    "О1 · переделать план",
    "Р3 · новый план утверждён",
    "Д · проверка пройдена (повторять)",
    "Б1 · повтор П2 — только в ветке без блока",
]
...
with gr.Blocks(title="TooManyRules") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            # Второй ряд кнопок задачи (день 15, §7.1): назад по графу. «Вернуться
            # к плану» и «Переоткрыть задачу» — тот же `on_task_event`, что пауза
            # и отмена; активны всегда: недопустимое нажатие отклоняет автомат, и
            # в статусе видно почему (это часть демонстрации красного пути).
            with gr.Row():
                back_to_plan_btn = gr.Button("↩ Вернуться к плану", size="sm", scale=1)
                reopen_task_btn = gr.Button("↩ Переоткрыть задачу", size="sm", scale=1)
            ...
            # иначе любое событие затирало бы несохранённую правку. С дня 15
            # свёрнут (§7.5, «на экране — текущий день»): инструмент прошлого
            # дня открывается одним кликом. Стоит под строкой статуса и над
            # закрытым редактором профиля.
            with gr.Accordion(
                "Инварианты — ограничения, которые задаёте вы (день 14)",
                open=False,
            ):
                gr.Markdown(
            ...
            with gr.Accordion("Примеры и сценарии прошлых дней (6, 10-15)", open=False):
                ...
                # «Отправить». С дня 15 свёрнут вместе с остальными прошлыми днями.
                gr.Examples(
        ...
        with gr.Column(scale=1):
            ...
            # инварианты встают в запрос раньше профиля. С дня 15 свёрнут
            # (§7.5): блок прошлого дня, тот же выход `_view()`.
            ...
            with gr.Accordion("Состояние задачи и маршрут (дни 13, 15)", open=False):
                ...
                task_md = gr.Markdown("")
                with gr.Accordion("Автомат задачи — таблица переходов", open=True):
    ...
    # Откаты назад по графу (день 15, §7.1) — тот же обработчик, новое событие.
    back_to_plan_btn.click(
        functools.partial(on_task_event, event=EVENT_BACK_TO_PLAN),
        inputs=[agent_state, preset_dropdown],
        outputs=COMMON_OUTPUTS,
    )
    reopen_task_btn.click(
        functools.partial(on_task_event, event=EVENT_REOPEN),
        inputs=[agent_state, preset_dropdown],
        outputs=COMMON_OUTPUTS,
    )
