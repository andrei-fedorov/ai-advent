...
class ContextTask:
    ...
    max_tokens: int | None
    covers: int               # сколько сообщений истории покроет результат
...
class SummaryStrategy:
    ...
    Промпт свёртки и все четыре числа приходят снаружи (из `presets.py`):
    про Too Many Bones этот модуль не знает.
    """
    ...
    def __init__(
        ...
        self.name = name
        self._prompt = prompt
        # Чётное `keep_last` — требование спецификации (§3.1): стек растёт
        # парами «вопрос + ответ», и срез из чётного числа сообщений всегда
        # начинается с `user`. Половинок ходов в запросе не бывает.
